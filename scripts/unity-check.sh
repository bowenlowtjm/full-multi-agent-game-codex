...
run_compile() {
  ...
  set +e
  if [ -n "$method" ]; then
    "$UNITY_BIN" -batchmode -quit -projectPath "$PROJ" -executeMethod "$method" -logFile Logs/compile.log
  else
    "$UNITY_BIN" -batchmode -quit -projectPath "$PROJ" -logFile Logs/compile.log
  fi
  CODE=$?
  set -e
}

# Prefer AutoRefresher when available; Unity 6 may not expose it in batch mode early in boot.
run_compile "AutoRefresher.RefreshAndExit"
if grep -q "executeMethod class 'AutoRefresher' could not be found" Logs/compile.log; then
  echo "[unity-check] AutoRefresher unavailable in this editor version/startup path; retrying plain batch compile"
  run_compile ""
fi

ERRS="$(grep -E 'error CS[0-9]+' Logs/compile.log || true)"
if [ -n "$ERRS" ] || grep -q "Scripts have compiler errors" Logs/compile.log || [ "$CODE" -ne 0 ]; then
